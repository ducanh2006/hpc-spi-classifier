...
int main(int argc, char **argv)
{
	...
	ret = rte_eth_rx_queue_setup(portid, 0, 1024, rte_eth_dev_socket_id(portid), NULL, mbuf_pool);
	if (ret < 0)
	...
	RTE_LCORE_FOREACH_WORKER(lcore_id) {
		if (num_workers >= MAX_WORKERS) {
			printf("Warning: More lcores than MAX_WORKERS (%d), ignoring lcore %u\n", MAX_WORKERS, lcore_id);
			continue;
		}

		char ring_name[32];
		snprintf(ring_name, sizeof(ring_name), "WORKER_RING_%u", num_workers);
		worker_rings[num_workers] = rte_ring_create(ring_name, RING_SIZE,
		...
		rte_eal_remote_launch(worker_loop, &w_params[num_workers], lcore_id);
		num_workers++;
	}

	if (num_workers == 0) {
		rte_exit(EXIT_FAILURE, "Need at least 1 worker core (e.g. -l 0-1)\n");
	}

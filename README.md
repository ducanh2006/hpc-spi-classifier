...
3.  **Ghi nhận kết quả phân loại chi tiết**:
    ...
    *   Tệp `actual.csv` này sau đó sẽ được so khớp trực tiếp với tệp kết quả mong đợi (`expected.csv`) để kiểm tra độ chính xác phân loại của thuật toán so khớp.Dưới đây là toàn bộ nội dung file README của bạn đã được dịch sang tiếng Việt, sử dụng các thuật ngữ chuyên ngành Công nghệ thông tin, Mạng máy tính và Điện toán hiệu năng cao (HPC) một cách chuẩn xác và mượt mà:
